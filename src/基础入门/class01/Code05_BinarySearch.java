...
public class Code05_BinarySearch {
    ...
    public static int process(int[] arr, int l, int r) {
        if (l == r && arr[l] < arr[l - 1] && arr[l] < arr[l + 1]) {
            return l;
        } else if (l == r - 1) {
            if (arr[r] < arr[r - 1] && arr[r] < arr[r + 1]) {
                return r;
            } else if (arr[l] < arr[l - 1] && arr[l] < arr[l + 1]) {
                return l;
            }
        } else if (l < r - 1) {
            int mid = l + ((r - l) >> 1);
            if (arr[mid] > arr[mid - 1]) {
                ...
                return process(arr, l, mid - 1) != -1 ? process(arr, l, mid - 1) : process(arr, mid + 1, r) == -1 ? -1 : process(arr, mid + 1, r);
            }
        }
        return -1;
    }
